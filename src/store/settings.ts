...
export const SettingsManager = {
  ...
  async getMyLanguage(): Promise<Language | null> {
    try {
      const raw = await RealmSettingsManager.getSetting(
  ...
  async getMyOTTs(): Promise<
    Array<{id: number; provider_name: string; logo_path?: string}>
  > {
    try {
  ...
  async setMyOTTs(
    providers: Array<{id: number; provider_name: string; logo_path?: string}>,
  ): Promise<void> {
    try {
      await RealmSettingsManager.setSetting(
        '@settings/my_otts',
  ...
  async getRegion(): Promise<any> {
    const region = await RealmSettingsManager.getSetting(
  ...
  async getContentLanguages(): Promise<Language[]> {
    try {
      const savedLanguages = await RealmSettingsManager.getSetting(
        '@settings_content_languages',
      );
