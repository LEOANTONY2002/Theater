...
import {useSimilarTVShows} from '../hooks/useTVShows';
...
type SimilarTVShowsNavigationProp =
  NativeStackNavigationProp<HomeStackParamList>;
type SimilarTVShowsRouteProp = RouteProp<HomeStackParamList, 'SimilarTVShows'>;

export const SimilarTVShowsScreen: React.FC = () => {
  return <SimilarTVShowsContent />;
};

const SimilarTVShowsContent = () => {
  const navigation = useNavigation<SimilarTVShowsNavigationProp>();
  const route = React.useRef(
    navigation.getState().routes[navigation.getState().index],
  ).current;
  const {tvId, title} = (route?.params as any) || {};
  ...
  const allItems = useMemo(
    () =>
      data?.pages.flatMap(page =>
        (page?.results || []).map((s: TVShow) => ({...s, type: 'tv' as const})),
      ) || [],
    [data],
  );
  ...
  const renderItem = ({item}: {item: ContentItem}) => (
    <MovieCard
      item={item}
      onPress={show => navigation.push('TVShowDetails', {show: show as TVShow})}
      cardWidth={cardWidth}
    />
  );
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>No similar TV shows found</Text>
            </View>
          }
        />
      )}
