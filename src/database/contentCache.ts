  ...
  try {
    ...
    realm.write(() => {
      realm.create('Movie', {
        ...
        crew: crew ? JSON.stringify(crew.filter((c: any) => 
          ['Director', 'Producer', 'Writer', 'Screenplay'].includes(c.job)
        )) : existing?.crew,
        // Videos
        trailer_key: videos?.find((v: any) => v.type === 'Trailer' && v.official)?.key || existing?.trailer_key,
...
export const cacheMovieAI = (
  ...
  try {
    ...
    if (!movie) {
      console.warn('[ContentCache] Movie not found, cannot cache AI data:', movieId);
      return;
    }
...
export const cacheTVShowAI = (
  ...
  try {
    ...
    if (!show) {
      console.warn('[ContentCache] TV show not found, cannot cache AI data:', showId);
      return;
    }
    ...
    realm.write(() => {
      shows.forEach(show => {
        try {
          ...
          console.error(`[ContentCache] Error caching TV show ${show.id}:`, itemError);
        }
      });
    });
  } catch (error) {
